...
import { PaperClipIcon, PhotoIcon, FaceSmileIcon, HandThumbUpIcon, PaperAirplaneIcon, XCircleIcon } from '@heroicons/react/24/outline';
...
export default function MessageInput({ conversation = null }) {
    ...
    const onSendClick = () => {
        ...
        if (newMessage.trim() === '' && chosenFiles.length === 0) {
            setInputErrorMessage("Provide a message or upload attachment");
    ...
    const removeFile = (fileName) => {
        setChosenFiles((prevFiles) => prevFiles.filter(f => f.file.name !== fileName));
    };
        ...
        <div className="flex flex-wrap items-start border-t border-slate-700 py-3">
            <div className="order-2 flex-1 xs:flex-none xs:order-1 p-2">
                ...
                <button className="p-1 text-gray-400 hover:text-gray-300 relative">
                    ...
                    <input type="file" onChange={onFileChange} multiple accept="images/*" className="absolute left-0 top-0 right-0 bottom-0 z-20 opacity-0 cursor-pointer" />
                </button>
            </div>
            <div className="order-1 px-3 xs:p-0 min-w[220px] basis-full xs:basis-0 xs:order-2 flex-1 relative">
                <div className="flex">
                    <NewMessageInput value={newMessage} onSend={onSendClick} onChange={(ev) => setNewMessage(ev.target.value)} />
                    <button className="btn btn-info rounded-1-none" onClick={onSendClick} disabled={messageSending}>
                        {messageSending && (
                ...
                {inputErrorMessage && (
                    <div className="text-xs text-red-400">{inputErrorMessage}</div>
                )}
                <div className="flex flex-wrap gap-1 mt-2">
                    {chosenFiles.map(file => (
                        <div key={file.file.name} className={`relative flex justify-between cursor-pointer ${!isImage(file.file) ? " w-[240px]" : ""}`}>
                            ...
                            <button onClick={() => removeFile(file.file.name)} className="absolute w-6 h-6 rounded-full bg-gray-800 -right-2 -top-2 text-gray-300 hover:text-gray-100 z-10">

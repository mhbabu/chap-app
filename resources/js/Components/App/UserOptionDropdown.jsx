...
import { EllipsisVerticalIcon, LockClosedIcon, LockOpenIcon, ShieldCheckIcon, UserIcon } from '@heroicons/react/24/outline';
...
export default function UserOptionDropdown({ conversation }) {
    ...
    const changeUserRole = () => {
        ...
        axios.post(route("users.changeRole", conversation.id))
            .then((res) => {
    ...
    const onBlockUser = () => {
        ...
        axios.post(route("users.userBlockUnblock", conversation.id))
            .then((res) => {
                console.log(res.data);
            }).catch((error) => {
                console.log(error);
